		...
		return (
			<motion.g
				...
				onKeyDown={(e: React.KeyboardEvent) => {
					if (e.key === "Enter" || e.key === " ") onMealClick?.(shape.id)
				}}
			>
				...
				<g className="text-muted/60" aria-hidden="true">
					{Array.from({ length: Math.min(shape.itemCount, 8) }).map((_, idx) => {
						...
						return (
							<line
								key={idx}
								x1={notchX}
								y1={shape.centerY + shape.height / 2 + 10}
								x2={notchX}
								y2={shape.centerY + shape.height / 2 + 16}
								stroke={color}
								strokeWidth="1"
							/>
						)
					...
					{shape.itemCount > 8 && (
						<text
							x={shape.centerX + 18}
							y={shape.centerY + shape.height / 2 + 16}
							className="fill-current font-mono text-[9px] font-bold"
						>
							+{shape.itemCount - 8}
						</text>
					)}
				...
				<text
					...
					textAnchor="middle"
					className="text-2xs fill-current font-mono font-bold"
					style={{ fill: color }}
				>
				...
				{shape.localTime && (
					<text
						x={shape.centerX}
						y={shape.centerY - shape.height / 2 - 2}
						textAnchor="middle"
						className="text-muted fill-current font-mono text-[8px] opacity-70"
					>
						{shape.localTime}
					</text>
				)}
...
export function DayImprintAnimated({
	scene,
	highlightedMealId,
	onMealClick,
}: DayImprintAnimatedProps) {
	return (
		<div className="relative w-full">
			<svg
				viewBox={VIEWBOX}
				className="border-subtle bg-surface h-auto w-full rounded-xl border select-none"
			...
			>
				...
				{!scene.suppressTimeAxis &&
					scene.timeTicks.map((tick) => (
						<g key={tick.minute} className="text-muted/60 font-mono" aria-hidden="true">
							...
							<text
								...
								textAnchor="middle"
								className="text-2xs fill-current font-bold"
							>
								{tick.label}
							</text>
						</g>
