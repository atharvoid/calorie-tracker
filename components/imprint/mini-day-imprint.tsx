...
export function MiniDayImprint({
	summary,
	mealGroups,
	width = 60,
	height = 32,
	...
	if (scene.state === "empty") {
		return (
			<div
				className="border-subtle bg-elevated/20 flex items-center justify-center rounded border"
				style={{ width, height }}
			>
				<span className="bg-muted/40 h-1.5 w-1.5 rounded-full" />
			</div>
		)
	}
	...
	return (
		<svg
			viewBox={VIEWBOX}
			style={{ width, height }}
			className="border-subtle bg-surface shrink-0 rounded border select-none"
