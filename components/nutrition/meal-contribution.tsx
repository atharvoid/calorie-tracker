...
function MealTooltip({ active, payload, label }: TooltipProps): React.ReactElement | null {
	...
	return (
		<div style={TOOLTIP_BOX}>
			...
			<div>
				{item.kcal.toLocaleString("en-IN")} kcal · {item.pct}%
			</div>
		</div>
	)
}
		...
		<div>
			<ResponsiveContainer width="100%" height={180}>
				<BarChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
					...
					<XAxis dataKey="mealType" tick={AXIS_TICK} tickLine={false} axisLine={false} />
					<YAxis
						tick={AXIS_TICK}
						tickLine={false}
						axisLine={false}
					...
					<Bar dataKey="kcal" radius={[6, 6, 0, 0]}>
						{data.map((entry) => (
							<Cell key={entry.mealType} fill={MEAL_COLORS[entry.mealType] ?? "#6B6C72"} />
						))}
					</Bar>
				</BarChart>
			</ResponsiveContainer>
