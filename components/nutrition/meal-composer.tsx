...
export function MealComposer({
  ...
  return (
    <div className="fixed inset-0 z-50 md:relative md:inset-auto md:z-0 flex flex-col bg-background md:bg-transparent min-h-dvh md:min-h-0 overflow-hidden md:overflow-visible">
          ...
          {!preview ? (
            <div className="space-y-2 flex flex-col">
              <textarea
                ...
                className="w-full min-h-[140px] rounded-lg border border-subtle bg-surface p-3 text-base text-primary placeholder-muted focus:border-accent focus:outline-none disabled:opacity-50 transition-all resize-none"
                autoFocus
              />
              {text.length > 400 && (
            ...
            <div className="rounded-lg border border-subtle bg-elevated/10 p-4 space-y-4">
                  ...
                  <div key={mIdx} className="space-y-1.5">
                    ...
                    <ul className="space-y-1">
                      {meal.items.map((item, iIdx) => (
                        <li
                          key={iIdx}
                          className="text-xs text-secondary flex flex-col sm:flex-row sm:items-center justify-between border-b border-subtle/50 pb-1.5 last:border-0"
                        >
              ...
              <div className="border-t border-subtle pt-3 flex flex-wrap items-center justify-between gap-2 text-xs font-semibold text-primary font-mono bg-elevated/20 p-2.5 rounded-lg">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-10 border-t border-subtle bg-surface/95 backdrop-blur-md px-4 py-3 md:relative md:bottom-auto md:left-auto md:right-auto md:border-0 md:bg-transparent md:p-0 md:pt-4 md:mt-2 flex items-center justify-end gap-2 pb-[calc(12px+env(safe-area-inset-bottom))] md:pb-0">
        <button
          type="button"
          onClick={onCancel}
          disabled={loading || committing}
          className="flex-1 md:flex-initial rounded-lg border border-subtle bg-surface px-4 py-2.5 text-xs font-semibold text-secondary hover:text-primary hover:bg-elevated disabled:opacity-50 transition-all text-center h-10 flex items-center justify-center cursor-pointer"
        >
          Cancel
        </button>
        ...
        {!preview ? (
          <button
            ...
            className="flex-1 md:flex-initial flex items-center justify-center gap-1.5 rounded-lg bg-accent px-4 py-2.5 text-xs font-semibold text-white hover:bg-accent-hover disabled:opacity-50 cursor-pointer h-10 transition-all"
          >
            {loading ? (
          ...
          <button
            ...
            onClick={handleSave}
            disabled={committing}
            className="flex-1 md:flex-initial flex items-center justify-center gap-1.5 rounded-lg bg-paid px-4 py-2.5 text-xs font-semibold text-white hover:bg-paid-hover disabled:opacity-50 cursor-pointer h-10 transition-all"
          >
            {committing ? (
              <Loader2 className="h-3.5 w-3.5 animate-spin" />
            ) : (
