...
export function MealComposer({
	...
	return (
		<div className="bg-background fixed inset-0 z-50 flex min-h-dvh flex-col overflow-hidden md:relative md:inset-auto md:z-0 md:min-h-0 md:overflow-visible md:bg-transparent">
			...
			<div className="bg-canvas/40 md:bg-surface border-subtle md:rounded-card flex-1 overflow-y-auto border p-5 pb-28 shadow-lg md:relative md:pb-0">
				<button
					onClick={onCancel}
					className="text-muted hover:text-primary hover:bg-elevated focus-visible:ring-accent ease-premium absolute top-3 right-3 flex h-11 w-11 cursor-pointer items-center justify-center rounded-full transition-colors focus-visible:ring-2 focus-visible:outline-none"
					aria-label="Close"
				>
					<X className="h-5 w-5" />
				</button>
			...
			<div className="border-subtle bg-surface/95 fixed right-0 bottom-0 left-0 z-10 flex items-center justify-end gap-2 border-t px-4 py-3 pb-[calc(12px+env(safe-area-inset-bottom))] backdrop-blur-md md:relative md:right-auto md:bottom-auto md:left-auto md:mt-2 md:border-0 md:bg-transparent md:p-0 md:pt-4 md:pb-0">
				<button
					...
					onClick={onCancel}
					disabled={loading || committing}
					className="border-subtle bg-surface text-secondary hover:text-primary hover:bg-elevated flex h-11 flex-1 cursor-pointer items-center justify-center rounded-lg border px-4 py-2.5 text-center text-xs font-semibold transition-all disabled:opacity-50 md:flex-initial"
				>
					Cancel
				</button>
				...
				{!preview ? (
					<button
						...
						className="bg-accent hover:bg-accent-hover flex h-11 flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-lg px-4 py-2.5 text-xs font-semibold text-white transition-all disabled:opacity-50 md:flex-initial"
					>
						{loading ? (
					...
					<button
						...
						onClick={handleSave}
						disabled={committing}
						className="bg-paid hover:bg-paid-hover flex h-11 flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-lg px-4 py-2.5 text-xs font-semibold text-white transition-all disabled:opacity-50 md:flex-initial"
					>
						{committing ? (
							<Loader2 className="h-3.5 w-3.5 animate-spin" />
						) : (
