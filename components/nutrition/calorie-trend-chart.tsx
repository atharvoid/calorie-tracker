...
function CaloriesTooltip({ active, payload, label }: TooltipProps): React.ReactElement | null {
	...
	return (
		<div style={TOOLTIP_BOX}>
			...
			{kcal === null && <div style={{ color: "#6B6C72" }}>No data</div>}
			{target !== null && (
				<div style={{ color: PENDING }}>Target: {target.toLocaleString("en-IN")} kcal</div>
			)}
			{maintenance !== null && (
				<div style={{ color: MUTED_LINE }}>
					Maintenance: {maintenance.toLocaleString("en-IN")} kcal
				</div>
			)}
		</div>
	)
...
export function CalorieTrendChart({ data, loading = false }: Props) {
	if (loading) {
		return <div className="bg-elevated h-[220px] animate-pulse rounded-xl" />
	}
		...
		<div>
			<ResponsiveContainer width="100%" height={220}>
				<AreaChart data={data} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
					...
					<Area type="monotone" dataKey="targetKcal" stroke="none" fill="none" />
					<Area type="monotone" dataKey="maintenanceKcal" stroke="none" fill="none" />
				</AreaChart>
			</ResponsiveContainer>
