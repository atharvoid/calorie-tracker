		...
		return (
			<form action={doSignIn}>
				<button
					type="submit"
					className={cn(
						"rounded-btn flex cursor-pointer items-center gap-2 px-3.5 py-2 text-xs font-semibold whitespace-nowrap shadow-sm transition-all hover:shadow sm:text-sm",
						SECONDARY_BTN
					)}
				>
	...
	return (
		<div className="flex items-center gap-3">
			...
			<div className="hidden flex-col sm:flex">
				<span className="text-primary text-xs leading-none font-medium">{session.user.name}</span>
				<span className="text-muted mt-0.5 text-xs leading-none">{session.user.email}</span>
			</div>
