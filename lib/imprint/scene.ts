...
export function buildScene(input: DayImprintInput): DayImprintScene {
	...
	if (input.meals.length === 0 && totals !== null) {
		...
		const { proteinShare, carbShare, fatShare } = computeMacroShares(
			totals.proteinG,
			totals.carbsG,
			totals.fatG
		)
		...
		const state =
			target.targetKcal === null
				? "partial"
				: consumed > target.targetKcal
					? "over-target"
					: "targeted"
	...
	if (prePlacedMeals.length > 8) {
		...
		const avgMinute = Math.round(
			excessMeals.reduce((sum, m) => sum + m.resolvedMinute, 0) / excessMeals.length
		)
	...
	prePlacedMeals.forEach((meal) => {
		...
		for (let j = 0; j < contourCount; j++) {
			const scale = contourCount > 1 ? 1.0 - (1.0 - 0.28) * (j / (contourCount - 1)) : 1.0
			contours.push(
				generateContourPath(centerX, centerY, meal.width, meal.height, scale, carbShare, prng)
			)
		}
