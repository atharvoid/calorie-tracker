...
export function resolveMinutes(
	meals: Array<{ mealType: string | null; localTime: string | null }>
): number[] {
	const result: number[] = []
...
export function generateContourPath(
	...
	for (let i = 0; i < pointCount; i++) {
		...
		const baseNoise =
			1 + 0.055 * Math.sin(3 * theta + phaseA) + 0.035 * Math.sin(5 * theta + phaseB) + jitter

		let x =
			centerX +
			Math.cos(theta) * (width / 2) * scale * baseNoise +
			carbSkew * Math.pow(Math.sin(theta), 2) * scale
