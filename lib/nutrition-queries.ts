...
import { db } from "@/db"
import { nutritionSettings, nutritionDayOverrides, mealItems } from "@/db/schema"
import { z } from "zod"
...
export async function upsertSettings(
	...
	if (existing) {
		const [updated] = await db
			...
			.set({
				maintenanceKcal:
					data.maintenanceKcal !== undefined ? data.maintenanceKcal : existing.maintenanceKcal,
				targetKcal: data.targetKcal !== undefined ? data.targetKcal : existing.targetKcal,
				proteinTargetG:
		...
		.values({
			...
			carbsTargetG:
				data.carbsTargetG !== null && data.carbsTargetG !== undefined
					...
					: null,
			fatTargetG:
				data.fatTargetG !== null && data.fatTargetG !== undefined ? String(data.fatTargetG) : null,
...
export async function getDayOverride(
	...
	const [row] = await db
		...
		.where(and(eq(nutritionDayOverrides.userId, userId), eq(nutritionDayOverrides.date, date)))
		.limit(1)
	return row ?? null
}
...
export async function deleteDayOverride(userId: string, date: string): Promise<void> {
	await db
...
export async function getMealItemsForRange(
	...
	return db
		.select()
		.from(mealItems)
		.where(and(eq(mealItems.userId, userId), gte(mealItems.date, start), lte(mealItems.date, end)))
			...
			grams:
				data.grams !== undefined
					? data.grams !== null
						...
						: null
					: existing.grams,
			kcal: data.kcal !== undefined ? String(data.kcal) : existing.kcal,
...
export async function deleteMealItem(userId: string, id: string): Promise<void> {
	const [existing] = await db
	...
	await db.delete(mealItems).where(and(eq(mealItems.id, id), eq(mealItems.userId, userId)))
}
