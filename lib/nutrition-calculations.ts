...
export function resolveGoal(
	...
	const hasSettings =
		settings !== null && (settings.maintenanceKcal !== null || settings.targetKcal !== null)
	...
	if (override) {
		const targetKcal = override.targetKcal ?? settings?.targetKcal ?? null
		const maintenanceKcal = override.maintenanceKcal ?? settings?.maintenanceKcal ?? null
	...
	return {
		...
		targetKcal,
		proteinTargetG:
			settings!.proteinTargetG !== null ? numericToNumber(settings!.proteinTargetG) : null,
		carbsTargetG: settings!.carbsTargetG !== null ? numericToNumber(settings!.carbsTargetG) : null,
		fatTargetG: settings!.fatTargetG !== null ? numericToNumber(settings!.fatTargetG) : null,
		toleranceKcal,
		source: "default",
	}
}

function computeTolerance(targetKcal: number | null, settings: DbSettings | null): number | null {
...
export function computeRemaining(
	...
	const targetDelta = goal.targetKcal !== null ? totals.kcal - goal.targetKcal : null

	const remainingToTarget =
		goal.targetKcal !== null ? Math.max(0, goal.targetKcal - totals.kcal) : null
...
export function computeDailySummary(
	...
	const assumptionCount = items.filter(
		(it) => it.notes !== null && it.notes.length > 0 && /assumed|approx|portion/i.test(it.notes)
	).length
...
export function computeWeeklyAverage(days: DailyNutritionSummary[]): {
	...
	const totalKcal = days.reduce((sum, d) => sum + (d.totals?.kcal ?? 0), 0)
	const coveragePercent = rangeDays > 0 ? Math.round((loggedDays / rangeDays) * 100) : 0
	return {
...
export function computeInsights(summary: DailyNutritionSummary): string[] {
	...
	const { totals, goal, targetDelta, maintenanceBalance, assumptionCount } = summary
	...
	if (goal.targetKcal !== null && targetDelta !== null) {
		...
		if (summary.status === "over") {
			insights.push(`You're ${abs} kcal over your ${goal.targetKcal} kcal target today.`)
		} else if (summary.status === "under") {
			insights.push(
				`You're ${abs} kcal under your ${goal.targetKcal} kcal target — consider a small meal or snack.`
			)
		} else {
			insights.push(`You're within your ${goal.targetKcal} kcal target — great consistency!`)
		}
	}
	...
	if (insights.length < 3 && maintenanceBalance !== null && goal.maintenanceKcal !== null) {
