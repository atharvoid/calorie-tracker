...
| S-4 | Add `UNIQUE(user_id)` to `sheet_connection` — duplicate connections can currently accumulate. Deduplicate existing rows first.                                                                                                                                                                                                                                                                                             | dropped | low      |
...
| D-2  | Drop the `entry` and `sheet_connection` tables. Both confirmed to have zero code references outside `db/schema.ts` (the Sheets feature and its settings UI were removed in PR #35). Their Drizzle exports have been deleted. The tables still exist in the live database — run `pnpm drizzle-kit generate` to produce the DROP migration, export a backup of both tables first, then `pnpm drizzle-kit migrate`. | in progress | high   |
...
| D-6  | Pick one payment provider. Both `stripe` and `dodopayments` are installed with both `lib/stripe.ts` and `lib/dodo.ts` present. Delete the loser and its env vars.                                                                                                                                                                                                                                                | in progress | medium |
