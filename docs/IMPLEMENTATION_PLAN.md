...
| A-20 | Add `zod` input validation and explicit body size caps to every remaining route handler.                                                                                                                                                                                                                                           | done    | medium   |
...
| S-3 | Give `numeric` columns explicit precision and scale, e.g. `numeric(7,2)` for macros. Bare `numeric` is returned by Drizzle as a **string**, which is why `Number()` coercion is scattered across every consumer. Consider `integer` for `kcal` and `grams`.                                                                              | done    | medium   |
...
| S-5 | Consider `date` instead of `text` for `meal_item.date` and friends, so the database validates the format. Weigh against the existing string-arithmetic date utilities, which are deliberately string-based and well tested.                                                                                                              | done    | medium   |
...
| D-6  | Pick one payment provider. Both `stripe` and `dodopayments` are installed with both `lib/stripe.ts` and `lib/dodo.ts` present. Dodo is the live provider; Stripe is the one to remove. Tracked as issue #46.                                                                                                                                                                                                                                                                                                  | done   | medium |
