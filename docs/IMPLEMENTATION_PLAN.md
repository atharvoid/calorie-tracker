...
| S-1 | `meal_item.capture_id` is `text` but references `pending_capture.id`, which is `uuid` — a type mismatch with no FK constraint at all. Migrate to `uuid` with a real FK: `ALTER TABLE meal_item ALTER COLUMN capture_id TYPE uuid USING capture_id::uuid;` then add `REFERENCES pending_capture(id) ON DELETE SET NULL`. **Verify every existing value casts cleanly first** — a single malformed row aborts the migration. Migration written as `0005_capture_id_uuid_fk.sql`; confirm it is applied. | todo    | **high** |
| S-2 | Convert the `$type<>()` unions above into real Postgres enums or `CHECK` constraints so the database enforces them, not just TypeScript. Migration written as `0006_column_constraints.sql`; confirm it is applied.                                                                                                                                                                                                                                                                                   | todo    | medium   |
...
| D-6  | Pick one payment provider. Both `stripe` and `dodopayments` are installed with both `lib/stripe.ts` and `lib/dodo.ts` present. Dodo is the live provider; Stripe is the one to remove. Tracked as issue #9 and owned by the `feat/card-gated-trial` branch — do not edit `app/api/billing/*`, `lib/stripe.ts`, or `lib/dodo.ts` from another branch.                                                                                                                                                          | in progress | medium |
...
| E-11 | Flatten `tests/__tests__/` to `tests/`. **Done** — test files flattened to `tests/` and `pattern-observation.test.ts` renamed to `nutrition-pattern-observation.test.ts`. All 16 suites passing cleanly.                                                                                                                                                                                                                                                                 | done           | low    |
