...
export async function GET(req: NextRequest): Promise<NextResponse> {
	...
	const targetEligibleSummaries = loggedSummaries.filter((s) => s.goal.targetKcal !== null)
	...
	const averageToleranceKcal =
		toleranceSummaries.length > 0 ? Math.round(sumTolerance / toleranceSummaries.length) : null
	...
	const adherencePct =
		loggedSummaries.length > 0 ? Math.round((adherentDays / loggedSummaries.length) * 100) : null
	...
	for (const item of allItems) {
		...
		const kcal = numericToNumber(item.kcal) ?? 0
	...
	const topByKcal = [...allFoods].sort((a, b) => b.totalKcal - a.totalKcal).slice(0, 10)
	const topByProtein = [...allFoods].sort((a, b) => b.totalProteinG - a.totalProteinG).slice(0, 10)

	// Meal contribution
	const mealContribMap = new Map<string, number>()
	for (const item of allItems) {
		const key = item.mealType ?? "Other"
		mealContribMap.set(key, (mealContribMap.get(key) ?? 0) + (numericToNumber(item.kcal) ?? 0))
	}
